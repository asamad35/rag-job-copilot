...
const defaultTargetsPath = path.resolve("scripts/live-targets-greenhouse.json")
const bundlePath = path.resolve(".tmp/autofill-eval-bundle.js")
const reportPath = path.resolve(".tmp/live-forms-batch-report.json")
...
const run = async () => {
  ...
  console.log(
    `Targets: attempted=${aggregate.attemptedTargets}, successful=${aggregate.successfulTargets}, failed=${aggregate.failedTargets}`
  )
  console.log(
  ...
  for (const report of reports) {
    if (!report.ok) {
      console.log(`❌ ${report.target.name}: ${report.error}`)
      continue
    }

    const metrics = report.metrics
    console.log(
      `✅ ${report.target.name}: discovered=${metrics.totalDiscovered}, resolved=${metrics.resolved}, ambiguous=${metrics.ambiguous}, unresolved=${metrics.unresolved}, layer2Resolved=${metrics.layer2Resolved}, rate=${metrics.resolvedRate}`
    )
  }
